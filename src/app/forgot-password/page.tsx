...
export default function ForgotPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: resetErr } = await supabase.auth.resetPasswordForEmail(e1, {
        redirectTo,
      });
    ...
    >
      <div className="mx-auto w-full max-w-3xl px-5 sm:px-8 py-12">
        <div className="rounded-[3px] border border-black/10 bg-white shadow-sm overflow-hidden">
          <div className="p-7">
            <div className="text-[18px] font-semibold tracking-[-0.01em] text-[#2c2c2c]">Forgot password</div>
            <div className="mt-2 text-[13px] text-black/60">We’ll email you a link to reset your password.</div>
            ...
            {sentTo ? (
              <div className="mt-4 rounded-[3px] border border-black/10 bg-[#fcfbf9] px-4 py-3 text-[13px] text-black/70">
                Reset link sent to <span className="font-semibold text-[#2c2c2c]">{sentTo}</span>.
              </div>
            ) : null}
