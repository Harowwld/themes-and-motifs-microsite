...
export async function GET(req: Request) {
  ...
  const rawLimit = searchParams.get("limit") ?? "42";

  const limit = Math.max(1, Math.min(100, Number(rawLimit) || 42));
  ...
  try {
    ...
    const vendorAllItems = (vendors ?? []) as unknown as VendorWithCoverImage[];
