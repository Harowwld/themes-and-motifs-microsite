...
function isAllowedUrl(raw: string): { allowed: boolean; error?: string } {
  try {
    ...
    // ALLOW BOTH HTTP AND HTTPS for vendor images
    // Many legacy vendor sites still use HTTP.
...
export async function GET(req: NextRequest) {
  ...
  if (!raw) {
    // Check if it's passed as a path segment (though this route is at /api/image-proxy)
    return new Response("Missing URL parameter", { status: 400 });
  }

  if (raw.includes("drive.google.com/file/d/")) {
    raw = convertGoogleDriveUrl(raw);
  }

  const urlCheck = isAllowedUrl(raw);
  if (!urlCheck.allowed) {
    console.error("URL check failed:", urlCheck.error, raw);
  ...
  try {
    ...
    const upstream = await fetch(raw, {
      redirect: "follow", // Allow redirects but validate the final URL in a production app if needed
    ...
    if (!upstream.ok) {
      console.error("Upstream error:", upstream.status, upstream.statusText, raw);
    ...
    console.error("Proxy error:", error, raw);
