...
Deno.serve(async (req: Request) => {
  // Supabase Database Webhooks send POST requests with the new record payload
  if (req.method !== "POST") {
    return new Response("Method not allowed", { status: 405 });
  }

  // Read SMTP secrets from Supabase Edge Function environment
  ...
  const vendorId = record.vendor_id as number;
  const fromName = String(record.name ?? "");
  const fromEmail = String(record.email ?? "");
  const message = String(record.message ?? "");

  if (!vendorId || !fromName || !fromEmail || !message) {
    return new Response("Incomplete inquiry record", { status: 400 });
  }

  // Determine if this is a promo inquiry (message starts with "[Promo Inquiry:")
  const isPromoInquiry = message.startsWith("[Promo Inquiry:");

  // Extract promo title and clean message if applicable
  let promoTitle = "";
  let cleanMessage = message;
  if (isPromoInquiry) {
    const titleMatch = message.match(/^\[Promo Inquiry: (.+?)\]/);
    promoTitle = titleMatch?.[1] ?? "Untitled Promo";
    // Strip the header lines added by the promo inquiry route
    cleanMessage = message.replace(/^\[Promo Inquiry:[^\]]+\]\n\nVendor: [^\n]+\nVendor Email: [^\n]+\n\n/, "");
  }

  // Look up vendor contact email from Supabase
  ...
  if (vendorError || !vendor?.contact_email) {
    console.error("Could not fetch vendor contact email:", vendorError);
    return new Response("Vendor not found or no contact email", { status: 404 });
  }

  const businessName = String(vendor.business_name);
  const vendorEmail = String(vendor.contact_email);

  const subject = isPromoInquiry
    ? `New Promo Inquiry: ${promoTitle}`
...
<head>
  ...
  <title>${isPromoInquiry ? "New Promo Inquiry" : "New Inquiry"}</title>
  <style>
...
<body>
  <div class="wrapper">
    <table class="main-table" align="center">
      ...
      <tr>
        <td class="content">
          <h1 class="h1">${isPromoInquiry ? "New Promo Inquiry" : "New Inquiry Received"}</h1>
          <div class="detail-row">
            <span class="detail-label">Vendor:</span>
            <span class="detail-value">${escapeHtml(businessName)}</span>
          </div>
          ${isPromoInquiry ? `<div class="detail-row"><span class="detail-label">Promo:</span><span class="detail-value">${escapeHtml(promoTitle)}</span></div>` : ""}
          <div class="detail-row">
